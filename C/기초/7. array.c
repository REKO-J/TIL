...
int main_array(void)
{
	...
	*/
	...
	{
		printf("%d\n", c_array[i]);  // 문자열 값
		printf("%c\n", c_array[i]);  // ASCII 코드 값(NULL 문자는 0으로 출력)
	}
	*/
	...
	*/
	...
	/*
	// 참고 1: ASCII 코드? ANSI(미국표준협회)에서 제시한 표준 코드 체계
	// 7bit, 총 128개 코드(0 ~ 127)
	// a: 97(문자 a의 아스키코드 정수 값)
	// A: 65
	// 0(숫자): 48
	printf("%c\n", 'a');  // 문자열 출력
	printf("%d\n", 'a');  // ASKII 코드 출력
	...
	*/
	...
	// 참고 2: 0 ~ 127 사이의 아스키코드 정수 값에 해당하는 문자 확인
	for (int i = 0; i < 128; i++)
	{
		printf("아스키코드 정수 %d: %c\n", i, i);
	}
