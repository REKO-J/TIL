...
def load_vocabulary(path, vocab_path, tokenize_as_morph=False):
    ...
    if not os.path.exists(vocab_path):
        ...
        if (os.path.exists(path)):
            ...
            # 공통적인 단어에 대해서는 모두 필요 없으므로 한개로 만들어 주기 위해서 set해주고 이것들을 리스트로 만들어 준다.
            words = list(set(words))
            # 데이터 없는 내용중에 MARKER를 사전에 추가 하기 위해서 아래와 같이 처리 한다.
            # 아래는 MARKER 값이며 리스트의 첫번째 부터 순서대로 넣기 위해서 인덱스 0에 추가한다.
            # PAD = "<PADDING>"
            # STD = "<START>"
            # END = "<END>"
            # UNK = "<UNKNWON>"
...
def dec_target_processing(value, dictionary, tokenize_as_morph=False):
    ...
    for sequence in value:
        ...
        # 문장 제한 길이보다 길어질 경우 뒤에 토큰을 자르고 있다. 그리고 END 토큰을 넣어 준다
        if len(sequence_index) >= MAX_SEQUENCE:
